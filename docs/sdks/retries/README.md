...
import { webhooksRetriesList } from "dwolla/funcs/webhooksRetriesList.js";
...
async function run() {
  const res = await webhooksRetriesList(dwolla, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("webhooksRetriesList failed:", res.error);
  }
}
