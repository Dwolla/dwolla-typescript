...
async function run() {
  const result = await dwolla.customer.transfers.list({
    id: "<id>",
  });
...
import { customerTransfersList } from "dwolla-typescript/funcs/customerTransfersList.js";
...
async function run() {
  const res = await customerTransfersList(dwolla, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("customerTransfersList failed:", res.error);
  }
}
