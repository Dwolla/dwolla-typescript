# CustomerMassPayments
(*customer.massPayments*)
...
async function run() {
  const result = await dwolla.customer.massPayments.list({
    id: "<id>",
  });
...
import { customerMassPaymentsList } from "dwolla-typescript/funcs/customerMassPaymentsList.js";
...
async function run() {
  const res = await customerMassPaymentsList(dwolla, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("customerMassPaymentsList failed:", res.error);
  }
}
