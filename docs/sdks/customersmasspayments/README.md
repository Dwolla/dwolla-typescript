...
import { customersMassPaymentsList } from "dwolla-typescript/funcs/customersMassPaymentsList.js";
...
async function run() {
  const res = await customersMassPaymentsList(dwolla, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("customersMassPaymentsList failed:", res.error);
  }
}
