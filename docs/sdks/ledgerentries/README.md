...
import { labelsLedgerEntriesList } from "dwolla/funcs/labelsLedgerEntriesList.js";
...
async function run() {
  const res = await labelsLedgerEntriesList(dwolla, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("labelsLedgerEntriesList failed:", res.error);
  }
}
...
async function run() {
  const result = await dwolla.labels.ledgerEntries.create({
    id: "<id>",
    requestBody: {
      amount: {
        value: "-5.00",
...
```typescript
import { DwollaCore } from "dwolla/core.js";
import { labelsLedgerEntriesCreate } from "dwolla/funcs/labelsLedgerEntriesCreate.js";
...
async function run() {
  const res = await labelsLedgerEntriesCreate(dwolla, {
    id: "<id>",
    requestBody: {
      amount: {
        value: "-5.00",
  ...
  if (res.ok) {
    ...
    console.log("labelsLedgerEntriesCreate failed:", res.error);
  }
}
...
<!-- UsageSnippet language="typescript" operationID="getLabelLedgerEntry" method="get" path="/ledger-entries/{ledgerEntryId}" -->
```typescript
import { Dwolla } from "dwolla";
...
```typescript
import { DwollaCore } from "dwolla/core.js";
import { labelsLedgerEntriesGet } from "dwolla/funcs/labelsLedgerEntriesGet.js";
...
async function run() {
  const res = await labelsLedgerEntriesGet(dwolla, {
    ledgerEntryId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("labelsLedgerEntriesGet failed:", res.error);
  }
}
