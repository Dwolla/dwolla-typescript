...
import { fundingSourcesBalanceGet } from "dwolla/funcs/fundingSourcesBalanceGet.js";
...
async function run() {
  const res = await fundingSourcesBalanceGet(dwolla, {
    id: "<id>",
  });
  if (res.ok) {
    ...
    console.log("fundingSourcesBalanceGet failed:", res.error);
  }
}
