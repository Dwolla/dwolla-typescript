/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v3";
import { remap as remap$ } from "../../lib/primitives.js";
import * as models from "../index.js";
import { DwollaError } from "./dwollaerror.js";

/**
 * Error returned when attempting to use a debit card funding source as the transfer source
 */
export type CardSourceNotAllowedErrorData = {
  code: models.CardSourceNotAllowedErrorCodeValidationError;
  message: string;
  embedded: models.CardSourceNotAllowedErrorEmbedded;
};

/**
 * Error returned when attempting to use a debit card funding source as the transfer source
 */
export class CardSourceNotAllowedError extends DwollaError {
  code: models.CardSourceNotAllowedErrorCodeValidationError;
  embedded: models.CardSourceNotAllowedErrorEmbedded;

  /** The original data that was passed to this error instance. */
  data$: CardSourceNotAllowedErrorData;

  constructor(
    err: CardSourceNotAllowedErrorData,
    httpMeta: { response: Response; request: Request; body: string },
  ) {
    const message = err.message || `API error occurred: ${JSON.stringify(err)}`;
    super(message, httpMeta);
    this.data$ = err;
    this.code = err.code;
    this.embedded = err.embedded;

    this.name = "CardSourceNotAllowedError";
  }
}

/** @internal */
export const CardSourceNotAllowedError$inboundSchema: z.ZodType<
  CardSourceNotAllowedError,
  z.ZodTypeDef,
  unknown
> = z.object({
  code: models.CardSourceNotAllowedErrorCodeValidationError$inboundSchema,
  message: z.string(),
  _embedded: z.lazy(() =>
    models.CardSourceNotAllowedErrorEmbedded$inboundSchema
  ),
  request$: z.instanceof(Request),
  response$: z.instanceof(Response),
  body$: z.string(),
})
  .transform((v) => {
    const remapped = remap$(v, {
      "_embedded": "embedded",
    });

    return new CardSourceNotAllowedError(remapped, {
      request: v.request$,
      response: v.response$,
      body: v.body$,
    });
  });
